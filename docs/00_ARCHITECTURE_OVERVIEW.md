...
1. **Scrape** → GitHub Actions (cron) runs Python script to fetch news from NewsAPI and RSS feeds.
...
3. **Transform (ELT)** → dbt (Data Build Tool) runs SQL in BigQuery to:
   ...
   - Aggregate and extract features (Gold layer: company entities, word counts).
